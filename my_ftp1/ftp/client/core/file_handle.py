...
class File_handle:
    def __init__(self,file_path):
        ...
        self.md5chk_alter = configuration['md5_check_alternation']
    ...
    def check_md5(self, check_size=None,start_position=0):
        m = hashlib.md5()
        block = int(check_size / self.md5chk_alter) if check_size > self.block_size * 10 else self.block_size
        with open(self.file_path, "rb") as f:
            ...
            while check_size:
                if check_size > block:
                    ...
                    m.update(content)
                    f.seek(f.tell() - self.block_size + block)
                    check_size -= block
                else:
                    f.read(check_size)
                    m.update(content)
