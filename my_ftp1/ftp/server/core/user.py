...
class UserInfo:
    ...
    user_info_path = (os.sep).join(server_path_lst)
    ...
    def load_user_info(cls):
        ...
        with open(cls.user_info_path,'r') as f:
            for info in f:
                ...
                root_path = '%s%s%s' % (cls.ftp_root, os.sep, username)
    ...
    def register(cls,usr,pwd):
        if usr in cls.user_dic.keys():
            ...
            with open(cls.user_info_path, 'a') as f:
                f.write('\n%s %s'%(usr,pwd))
            root_path = '%s%s%s'%(cls.ftp_root,os.sep,usr)
            os.mkdir(root_path)
